...
def build_loader(dataset, mode="train", batch_size=4, num_workers=4, mix_prob=0.0):

    if mode == "train" and mix_prob > 0:
        _collate_fn = partial(point_collate_fn, mix_prob=mix_prob)
    else:
...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        # Instantiate datasets for each split present in config
        for split in ["train", "val", "test"]:
            if split not in self.data_cfg:
                raise RuntimeError
            split_cfg = self.data_cfg[split]
            self.datasets[split] = hydra.utils.instantiate(split_cfg)

    def _make_dataloader(self, split):
        dl_cfg = self.data_cfg["dataloader"]
        return build_loader(
            self.datasets[split],
            mode=split,
            batch_size=dl_cfg.batch_size,
            num_workers=dl_cfg.num_workers,
            mix_prob=dl_cfg.mix_prob,
        )
